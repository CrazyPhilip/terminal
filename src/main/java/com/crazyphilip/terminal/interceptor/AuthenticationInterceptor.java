package com.crazyphilip.terminal.interceptor;
...
import com.crazyphilip.terminal.annotation.PassToken;
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object object) throws Exception {
        ...
        if (method.isAnnotationPresent(UserLoginToken.class)) {
            ...
            if (userLoginToken.required()) {
                ...
                // 获取 token 中的 user id
                String uid;
                System.out.println("token在此：" + token);
                try {
                    uid = JWT.decode(token).getAudience().get(0);
                } catch (JWTDecodeException j) {
                    throw new RuntimeException("401");
                }
                User user = userService.selectUsers(uid).get(0);
                if (user == null) {
